use crate::db::database::{
    clear_cells_by_column_from_db, delete_db_column_from_db, delete_db_column_option_from_db,
    delete_db_row_from_db, get_db_data_from_db, insert_db_cell_in_db, insert_db_column_into_db,
    insert_db_column_option_into_db, insert_db_row_into_db, update_db_column_in_db,
    update_db_column_option_in_db,
};
use crate::models::database::{
    DbCell, DbColumn, DbColumnOption, DbData, DbRow, NewDbCell, NewDbColumn, NewDbColumnOption,
    NewDbRow,
};
...
#[tauri::command]
pub fn create_column_option(
    db: tauri::State<AppDb>,
    data: NewDbColumnOption,
) -> AppResult<DbColumnOption> {
    insert_db_column_option_into_db(&db, data)
}

#[tauri::command]
pub fn update_column_option(
    db: tauri::State<AppDb>,
    id: i64,
    label: String,
    color: String,
    ...
    update_db_column_option_in_db(&db, id, label, color, order_index)
}

#[tauri::command]
pub fn delete_column_option(db: tauri::State<AppDb>, option_id: i64) -> AppResult<()> {
    delete_db_column_option_from_db(&db, option_id)
}
