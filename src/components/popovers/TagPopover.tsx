...
export default function TagPopover({ nestlingId }: { nestlingId: number }) {
  ...
  return (
    <>
      ...
      <label className="mb-2 flex items-center gap-1 text-xs font-medium text-zinc-700 dark:text-zinc-200">
        <TagIcon size={14} />
          ...
          {availableTags.map((tag) => (
            <NestlingTag
              ...
              removeIcon={
                <Trash2
                  size={12}
                  className="text-zinc-500 transition hover:text-red-500 dark:text-zinc-400 dark:hover:text-red-400"
                />
              }
