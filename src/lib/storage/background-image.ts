...
const LAST_BACKGROUND_KEY = "lastBackgroundImages";
const STORED_BACKGROUND_KEY = "storedBackgroundImages";

export async function getLastBackgroundImage(
  nestId: number | null,
): Promise<number | null> {
  if (nestId == null) return null;
  const map = await getMap<LastBackgroundImages>(LAST_BACKGROUND_KEY);
...
export async function saveLastBackgroundImage(nestId: number, imageId: number) {
  ...
  await saveMap<LastBackgroundImages>(LAST_BACKGROUND_KEY, map);
}
...
export async function getStoredBackgroundImage(
  ...
  if (nestId == null) return null;
  const map = await getMap<StoredBackgroundImages>(STORED_BACKGROUND_KEY);
  return map[nestId.toString()] ?? null;
}
...
export async function saveStoredBackgroundImage(
  ...
  const map = await getMap<StoredBackgroundImages>(STORED_BACKGROUND_KEY);
  map[nestId.toString()] = imageId;
  await saveMap<StoredBackgroundImages>(STORED_BACKGROUND_KEY, map);
}
